...
#include "modem_detect.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <glob.h>

static void detectEventProcess(void *data)
{
    modemHwInfo_t *hwInfo = (modemHwInfo_t*)data;
    int err = updateSystemNetworks(NULL,hwInfo->interface);
    if(err == EXIT_FAILURE){
        exit(0);
    }

    err = updateSystemLeds(hwInfo->interface);
    if(err == EXIT_FAILURE){
        exit(0);
    }
...
int modemInsertDetect() 
{
    ...
    for (size_t i = 0; i < glob_result.gl_pathc; ++i) {
        ...
        detectEventProcess(&modemHwInfo);
    }
